...
public class ResolvedTypeWithMembers
{
    ...
    protected ResolvedField[] resolveMemberFields()
    {
        ...
        for( int typeIndex = _types.length; --typeIndex >= 0; )
        {
            ...
            { // If actual type, add fields, masking whatever might have existed before:
    ...
    protected ResolvedField[] resolveStaticFields()
    {
        ...
        for( int typeIndex = _types.length; --typeIndex >= 0; )
        {
            ...
            if( thisType.isMixin() )
            {
                for( RawField raw : thisType.getType().getStaticFields() )
                {
                    if( ( _fieldFilter != null ) && !_fieldFilter.include( raw ) )
                    {
                        continue;
            ...
            { // If actual type, add fields, masking whatever might have existed before:
                for( RawField field : thisType.getType().getStaticFields() )
                {
                    if( ( _fieldFilter != null ) && !_fieldFilter.include( field ) )
                    {
                        continue;
            ...
            for( RawMethod method : type.getType().getMemberMethods() )
            {
                ...
                { // mix-in: only get annotations
                    for( Annotation ann : method.getAnnotations() )
                    {
                        ...
                        { // If no method, need to add to annotation override map
                    ...
                    if( old == null )
                    { // no method (yet), add argument annotations to override map
                        Annotations[] oldParamAnns = paramOverrides.get( key );
                        if( oldParamAnns == null )
                        { // no existing argument annotations for method
                    ...
                    { // already have a method, apply argument annotations as defaults
                        for( int i = 0; i < argAnnotations.length; i++ )
                        {
                            for( final Annotation annotation : argAnnotations[ i ] )
                            {
                ...
                { // "real" methods; add if not present, possibly add defaults as well
                    if( old == null )
                    { // new one to add
                    ...
                    { // method masked by something else? can only contribute annotations
                        for( Annotation ann : method.getAnnotations() )
                        {
                            if( methodCanInherit( ann ) )
                            {
